...
fn main() {
    ...
    let points_part_1 = extend_coords(&contents, false);
    let points_part_2 = extend_coords(&contents, true);

    println!("Part 1: {}", count_points(points_part_1));
    println!("Part 2: {}", count_points(points_part_2));
}

fn read_input() -> Coords {
...
fn count_points(points: Vec<Point>) -> usize {
    ...
    value_counts.into_iter().filter(|x| x.1 > 1).count()
}
...
fn is_vertical(pair: &Pair) -> bool {
    pair[0][0] == pair[1][0]
}

fn is_horizontal(pair: &Pair) -> bool {
    pair[0][0] == pair[1][0]
}
...
fn extend_diagonal_coords(pair: Pair) -> Vec<Point> {
    ...
    if x_start < x_end {
        x.append(&mut (x_start..=x_end).collect());
    } else {
        x.append(&mut (x_end..=x_start).rev().collect());
    }

    if y_start < y_end {
        y.append(&mut (y_start..=y_end).collect());
    } else {
        y.append(&mut (y_end..=y_start).rev().collect());
    }
...
fn test_extend_coords() {
    ...
    assert_eq!(
        extend_vertical_coords(vertical),
        vec![(1, 1), (1, 2), (1, 3)]
    );

    let vertical_rev: Pair = vec![vec![1, 3], vec![1, 1]];
    assert_eq!(
        extend_vertical_coords(vertical_rev),
        vec![(1, 3), (1, 2), (1, 1)]
    );

    let horizontal: Pair = vec![vec![7, 7], vec![9, 7]];
    assert_eq!(
        extend_horizontal_coords(horizontal),
        vec![(7, 7), (8, 7), (9, 7)]
    );

    let horizontal_rev: Pair = vec![vec![9, 7], vec![7, 7]];
    assert_eq!(
        extend_horizontal_coords(horizontal_rev),
        vec![(9, 7), (8, 7), (7, 7)]
    );

    let diagonal_up_right: Pair = vec![vec![1, 1], vec![3, 3]];
    assert_eq!(
        extend_diagonal_coords(diagonal_up_right),
        vec![(1, 1), (2, 2), (3, 3)]
    );

    let diagonal_up_left: Pair = vec![vec![9, 7], vec![7, 9]];
    assert_eq!(
        extend_diagonal_coords(diagonal_up_left),
        vec![(9, 7), (8, 8), (7, 9)]
    );
}
